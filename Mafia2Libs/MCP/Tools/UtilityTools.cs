...
public class UtilityTools
{
    ...
    [McpServerTool(Name = "get_configured_games"), Description("Get the list of games configured in MafiaToolkit with their installation paths")]
    public string GetConfiguredGames()
    {
        try
        {
            var gameStorage = GameStorage.Instance;
            var selectedGame = gameStorage.GetSelectedGame();

            var games = gameStorage.Games.Select(game =>
            {
                ...
                return new
                {
                    ...
                    isSelected = selectedGame != null && selectedGame.GameType == game.GameType
                };
            }).ToList();

            var configuredCount = games.Count(g => g.directoryExists);

            return JsonSerializer.Serialize(new
            {
                success = true,
                totalGames = games.Count,
                configuredGames = configuredCount,
                selectedGame = selectedGame != null ? new
                {
                    name = selectedGame.Name,
                    gameType = selectedGame.GameType.ToString(),
                    directory = selectedGame.Directory
                } : null,
                games
            });
        }
    ...
    {
        if (header.Length < 128) return "incomplete header";
