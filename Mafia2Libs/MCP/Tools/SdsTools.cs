...
public class SdsTools
{
        ...
        [Description("Game type hint: MafiaII, MafiaII_DE, MafiaIII, MafiaI_DE (optional)")] string? gameType = null)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(gameType) && Enum.TryParse<GamesEnumerator>(gameType, true, out var parsed))
            {
                game = parsed;
            }
            ...
            {
                success = true,
                file = new { path = info.FilePath, name = info.FileName, size = info.FileSize, sizeFormatted = FormatSize(info.FileSize) },
                header = new
                {
                ...
                resourceTypes = info.ResourceTypes.Select(rt => new { id = rt.Id, name = rt.Name, parent = rt.Parent }).ToList(),
    ...
    [McpServerTool(Name = "get_sds_header"), Description("Get SDS file header information")]
    public string GetSdsHeader([Description("Full path to the SDS file")] string filePath)
    {
        try
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                {
                    version = info.Version,
                    versionDescription = info.Version switch { 19 => "Mafia II", 20 => "Mafia III / Mafia I: DE", _ => "Unknown" },
                    platform = info.Platform.ToString(),
                    endian = info.Endian.ToString(),
                    slotRamRequired = info.SlotRamRequired,
    ...
    public string ListResources(
        ...
        [Description("Number of resources to return (default: 100)")] int limit = 100)
    {
        try
        {
            var info = _sdsService.OpenFile(filePath);
            ...
            var pageResources = resources.Skip(offset).Take(limit).Select(r => new
            {
                index = r.Index,
                typeId = r.TypeId,
                typeName = r.TypeName,
            ...
            {
                success = true,
                pagination = new { offset, limit, count = pageResources.Count, totalCount, hasMore = offset + pageResources.Count < totalCount },
                typeFilter,
    ...
    [McpServerTool(Name = "get_resource_info"), Description("Get detailed information about a specific resource")]
    ...
    public string SearchResources(
        [Description("Full path to the SDS file")] string filePath,
        [Description("Search pattern (case-insensitive)")] string pattern,
        [Description("Filter by resource type name")] string? typeFilter = null,
    ...
    [McpServerTool(Name = "extract_resource"), Description("Extract resource data as base64")]
    public string ExtractResource(
        [Description("Full path to the SDS file")] string filePath,
        [Description("Resource index (0-based)")] int resourceIndex,
        [Description("Max bytes to return (default: 1MB)")] int? maxBytes = null)
    {
        try
        {
    ...
    public string GetSdsStats([Description("Full path to the SDS file")] string filePath)
    {
        try
        {
            ...
            {
                success = true,
                file = new { name = info.FileName, size = info.FileSize, sizeFormatted = FormatSize(info.FileSize) },
                summary = new
                {
                    resourceCount = info.Resources.Count,
                    resourceTypeCount = info.ResourceTypes.Count,
