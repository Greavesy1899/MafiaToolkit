...
namespace ModelViewer.Programming.GraphicClasses
{
    public class TextureClass
    {
        ...
        public Texture2D LoadFromFile(Device device, ImagingFactory factory, string fileName)
        {
            ...
            //    bitmap.Save(SystemConfigClass.DataFilePath + "texture.bmp");
            //}
            //catch (Exception ex)
            //{
            //    Debug.WriteLine(ex.Message);
            //    Debug.WriteLine("Using game directory..");
            //    bitmap = DDS.LoadImage(SystemConfigClass.GamePath + fileName, false);
            //    bitmap.Save(SystemConfigClass.DataFilePath + "texture.bmp");
            //}
            using (var bs = LoadBitmap(factory, Mafia2Tool.ToolkitSettings.DataPath + "texture.bmp"))
